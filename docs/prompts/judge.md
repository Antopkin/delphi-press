...
- `a = 1.73` (√3, extremization: сдвигает вероятности от 0.5 к краям)
- `b = 0.0` (без систематического смещения)

Логическое обоснование: Baron et al. (2014) показали, что группы прогнозистов
дают оценку 0.65-0.70 там, где реальная частота событий — 0.80-0.85. AIA
Forecaster (2024, arXiv:2511.07678) — первая LLM-система, достигшая
суперпрогнозистского паритета (Brier 0.1076 vs 0.1110) — использует Platt
scaling с коэффициентом √3 ≈ 1.73. Neyman & Roughgarden (2021) теоретически
вывели оптимум для n=5 агентов: d ≈ 1.53.

ВАЖНО: параметр `a` конфигурируемый. При накоплении ≥30 resolved predictions
выполняй re-fit через grid search a∈[1.3, 2.2], минимизируя групповой Brier score.
...
3. Применяй temporal decay: оценки Раунда 2 получают множитель 1.5× по сравнению
   с Раундом 1 (Atanasov et al. 2017, Management Science). R2 содержит
   информацию из медиаторского синтеза и optional supervisor search — она
   объективно более информирована.
   Итоговая формула: `final_weight = brier_weight × temporal_decay × base_weight`
...
#### 4a. Штраф за длинный горизонт (>14 дней)

Для событий, где `days_until_target > 14` (от даты запуска пайплайна):
- Понизь `confidence_label` на одну ступень (высокая → умеренная, умеренная → низкая)
- Добавь флаг `long_horizon_penalty: true` в вывод

Обоснование: Ye et al. (2024, arXiv:2407.01231) показали на 991,759 GDELT-событиях
существенное падение точности LLM-прогнозов на горизонтах 30 и 90 дней vs. 1-дневного.
Mellers et al. (2014): суперпрогнозисты на 300-дневном горизонте ≈ обычные на 100 днях.

Исключение: события с фиксированной датой (выборы, заседание ЦБ, истечение соглашения) —
штраф применяется к confidence, но не к newsworthiness.
...
- a (extremization): {{ calibration_params.a | default(1.73) }}
- b (bias correction): {{ calibration_params.b | default(0.0) }}
{% else %}
Калибровочные параметры: используй значения по умолчанию (a=1.73, b=0.0).
{% endif %}
```
