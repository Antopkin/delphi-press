...
|---|---|---|
| Русский | Claude Sonnet (OpenRouter) | Надёжная мультиязычная генерация, хорошее понимание русского стиля |
...
class StyleReplicator(BaseAgent):
    ...
    Выбор модели: Claude Sonnet через OpenRouter для всех языков.
    """
...
Claude Sonnet используется для всех языков, включая русский -- модель хорошо понимает стилистические нюансы русскоязычных медиа (канцелярит, публицистические штампы, разговорный стиль Telegram-каналов и т.д.).
...
class QualityGate(BaseAgent):
    ...
    async def _check_style(
        ...
        Модель: Claude Sonnet через OpenRouter для всех языков.
        """
        model_tier = (
...
2. `StyleReplicator._select_model_tier()` выбирает модель:
   - `"russian"` -> Claude Sonnet (через OpenRouter)
...
| StyleReplicator | ~27 (9 predictions * 3 variants) | Claude Sonnet | $1.50 |
| QualityGate: factual | ~27 | Claude Sonnet | $1.00 |
| QualityGate: style | ~27 | Claude Sonnet | $0.80 |
| QualityGate: embeddings | ~1 batch | text-embedding-3-small | $0.01 |
| Ревизии (если нужны) | ~3-5 | Claude Sonnet | $0.20 |
