...
1. **Один LLM-клиент на все агенты**: `LLMClient` -- обёртка над OpenRouter/YandexGPT. Каждый агент использует его для вызовов, указывая модель в параметрах. Клиент мультиплексирует запросы к разным моделям через один HTTP-пул.
