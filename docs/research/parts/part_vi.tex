...
Источник: ${\sim}470$M ставок c Polymarket (HuggingFace datasets
\texttt{sandeepkumarfromin/full-market-data-from-polymarket} --- trades, CC0;
\texttt{ismetsemedov/polymarket-prediction-markets} --- markets + resolutions).
...
\footnote{Walk-forward evaluation использует relaxed порог $n_i \geq 5$ для удержания достаточного числа профилей в темпоральных сплитах.}
...
\begin{itemize}[nosep]
  ...
  \item $\widetilde{\bs}$ --- медианный BS по всей популяции на текущем датасете (${\approx}\,0.295$),
...
\midrule
INFORMED  & Top 20\% ($\bs \leq p_{20}$)        & 348\,519\textsuperscript{*} & Исторически точные \\
...
\textsuperscript{*}Количества на текущем датасете.}
\label{tab:tiers}
\end{table}
...
selection по AIC \citep{burnham2002model}:
\begin{equation}
\label{eq:weibull}
...
Walk-forward evaluation: burn-in 180 дней, шаг 60 дней, тестовое окно 60 дней ---
что даёт 22 фолда на текущем датасете.
