...
- **Cost tracking показывал $0 для всех стадий при использовании Haiku 4.5 / Gemini 2.5 Flash Lite** (`src/llm/pricing.py`). **Почему:** `MODEL_PRICING` dict не содержал entries для `anthropic/claude-haiku-4.5` и `google/gemini-2.5-flash-lite` — моделей, которые реально использовались в `scripts/eval_forecast_vs_reality.py` для forecast прогонов 2026-04-11. `calculate_cost()` для неизвестной модели silently возвращал `0.0`, поэтому все 18 стадий 3 прогонов зафиксировались с `cost_usd: 0.0`. Code path от `OpenRouterClient.complete()` через `BaseAgent.track_llm_usage()` в `StageResult.total_cost_usd` работает корректно — проблема только в lookup table. Добавлены entries (цены верифицированы на openrouter.ai: Haiku 4.5 `$1.00/$5.00`, Flash Lite `$0.10/$0.40` per M tokens). Добавлен warn-once logger в `calculate_cost()`, который логирует `logger.warning(...)` один раз per model для неизвестных ключей — silent failure превращается в заметное предупреждение, предотвращая повторение того же гэпа в будущем. Регрессионные тесты: `test_openrouter_claude_haiku_4_5`, `test_openrouter_gemini_2_5_flash_lite`, `test_unknown_model_logs_warning_once` в `tests/test_llm/test_pricing.py`.

- **Ruff F541 + stale baseline text в forecast_vs_reality runner** (`scripts/eval_forecast_vs_reality.py:521`). **Почему:** строка содержала f-string без `{}` placeholder (lint warning) и одновременно захардкоженную догадку `~0.10–0.15` про random-headline baseline, которая противоречила реально измеренному cross-outlet shuffled baseline `0.284` в том же meeting-артефакте `docs/meeting/forecast_vs_reality.md` (раздел "Honest Baseline Analysis"). При перегенерации артефакта через `--collect` получалось внутреннее противоречие. Заменено на текстовую строку (без `f` prefix) с реальными измеренными значениями: cross-outlet `0.284`, shuffled-words `0.264`, anti-baseline `0.235`.
