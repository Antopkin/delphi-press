---
title: "Pipeline: 9 стадий"
description: "Delphi Press реализует девятистадийный sequential-parallel hybrid конвейер: каждая стадия выполняется последовательно, но агенты внутри стадии могут работать параллельно (в зависимости от..."
---
...
В `DEFAULT_ASSIGNMENTS` (`src/llm/router.py`) зарегистрированы 28 задач. Judge — 28-я запись — имеет ассоциированную модель, но Stage 6 фактически выполняется детерминированно (market-weighted aggregation без LLM-вызова). Чистых LLM-инвокаций в pipeline — **27**.
