...
!!! info "Формат"
    ...
    Текущая версия: **v0.9.9** (2026-04-14). Тесты: 1 425.
...
| CWE-798 security fix | v0.9.9 | Blocklist + auto-generation + production enforcement, 9 TDD-коммитов, +12 тестов (1425 total) |
...
| P2 | Fix `MediaAnalyst.get_timeout_seconds()` override 300s | `media_analyst` тайм-аутится в sequential mode, стадия trajectory теряет одного аналитика. **Частично сделано (v0.9.9):** stage timeouts ↑ 1800s (`bc33ff2`, `cbc1107`). Per-agent override — TODO. | Низкая — override в dry_run.py |
...
!!! info "В процессе (2026-04-11–14)"
    Forward-forecast pilot запущен: 3 outlets (ТАСС / РИА / РБК) × `target_date=today` × Haiku 4.5. Predicted headlines (3 runs) лежат в `data/eval/forecast_runs/`, ждут Wayback индексации для сбора ground truth через `src/eval/ground_truth.py::fetch_headlines_from_wayback_html()`. Полный retrospective (50×3×Opus) — следующая фаза, требует ~$200.
...
**Контекст**: Stage 6 (Judge) генерирует `PredictedTimeline` — ранжированный список событий с вероятностями. In-memory объект уже есть: Judge возвращает `predicted_timeline` в `PipelineContext.predicted_timeline` (`src/schemas/pipeline.py:136-139`). Но **не сохраняется** в БД. Без persist невозможно сравнить *наш прогноз* с *рыночной ценой Polymarket* per-event (Delphi BS vs Market BS).

**Что делать**: JSON dump `PredictedTimeline` в `PipelineStep.output_data` для Stage 6 (поле уже есть в схеме, пустует). Добавить endpoint `GET /predictions/{id}/events` для просмотра.
...
**Контекст**: весь pipeline зависит от одного провайдера — OpenRouter. Если OpenRouter недоступен, ни один LLM-вызов не пройдёт. Это документировано как MEDIUM IMPACT gotcha. **Model-level fallback уже реализован** в `ModelRouter` (`src/llm/router.py:290-342`): Opus → Sonnet → Haiku при rate limit/ошибке. Этот пункт — про **provider-level** fallback.
...
**Сложность**: средняя. Нужен fallback по *провайдерам*, не по моделям. Требует отдельных API-ключей (`ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`).
...
**Контекст**: система определяет R1 vs R2 по наличию `mediator_synthesis` в контексте. Если Mediator (Stage 5) падает, R2 персоны думают, что они в R1 — получают неправильный промпт и не видят feedback других персон. HIGH IMPACT gotcha. **Частичная поддержка уже есть**: `AgentContext.round_number: Literal[1, 2]` существует (`src/schemas/agent.py:158`), используется в промптах. Задача — добавить поле в `PipelineContext` и проставлять явно в orchestrator, убрав implicit detection по `mediator_synthesis`.
...
**Минимальные критерии** (`[~]` — partial):

- [~] Retrospective Eval Pilot пройден (CompositeScore > baseline) — инфра готова, ждём Wayback ground truth
- [ ] Checkpoint & Resume реализован (cost recovery при timeout)
- [ ] QualityGate revision pipeline (не теряем заголовки)
- [~] Event-level storage (per-prediction market comparison) — in-memory готов, persist+API TODO
- [~] LLM provider fallback (не зависим от одного OpenRouter) — model-level готов, provider-level TODO
...
!!! info "Контекст"
    Раздел добавлен на основе литературного обзора 24 статей (апрель 2026). Реализация через Claude Code субагентов.
...
Детальное описание — в [Discussion RFC](discussion.md). Литературная валидация по 6 тезисам: 6 конспектов, 27K слов (локальные notes, не в репо).
...
Полные конспекты литобзора (24 файла, 84K слов), синтетический литобзор и аналитическая записка хранятся локально (gitignored, не в репо). Ключевые источники см. ниже.
