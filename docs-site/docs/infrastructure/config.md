...
description: "Delphi Press управляется единой системой конфигурации, объединяющей переменные окружения, файлы настроек и параметры пресетов. Все значения загружаются автоматически при старте приложения."
---
